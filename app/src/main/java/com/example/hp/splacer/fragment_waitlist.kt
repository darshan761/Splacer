 ...
 */
class fragment_waitlist : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater!!.inflate(R.layout.fragment_waitlist, container, false)

        viewManager = LinearLayoutManager(this.context)
        var mDatabase: DatabaseReference
        mDatabase = FirebaseDatabase.getInstance().reference

        var r = mDatabase.child("Waitlist").orderByChild("email").equalTo(mAuth.currentUser?.email.toString())
                    ...
                    o.addListenerForSingleValueEvent(object : ValueEventListener {
                        override fun onDataChange(p0: DataSnapshot?) {
                            ...
                            p0?.children?.forEach {
                                ...
                                myDataset.add(Com)
                                viewAdapter = MyAdapter2(myDataset)
